...
impl Mlsys {
    ...
    fn invoke_mlsys(&self, n_hosts: usize) -> io::Result<()> {
        ...
        //     "run ../data_test/checkpoints/model_llama_tc_bt10.bin ../data_test/checkpoints/model_mlp_tc_bt10.bin {data_dir} -b 10 -e 288 -n {n_hosts} -p 30 -t 10 > {data_dir}/output.txt 2>&1"
        // );
        let c_command = format!(
