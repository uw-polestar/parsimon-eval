...
const NR_PATHS_SAMPLED_NS3: usize = 2;
...
impl Experiment {
    ...
    fn run_ns3_path_all(&self, mix: &Mix) -> anyhow::Result<()> {
        ...
        // let mut path_list = path_to_flows_vec_sorted
        //     .choose_multiple(&mut rng, NR_PATHS_SAMPLED_NS3)
        //     .map(|x| x.0.clone())
        //     .collect::<Vec<_>>();
        ...
        if SAMPLE_MODE==1{
            ...
            let mut rng = StdRng::seed_from_u64(self.seed);
            flow_sampled_set=flowid_pool.choose_multiple(&mut rng, NR_PATHS_SAMPLED_NS3);
    ...
    fn run_flowsim(&self, mix: &Mix) -> anyhow::Result<()> {
        ...
        // let mut path = path_to_flowid_map
        //     .iter()
        //     .max_by_key(|x| x.1.len())
        //     .unwrap()
        //     .0;
        ...
        for src_dst_pair in path.iter() {
            if channel_to_flowid_map.contains_key(src_dst_pair) {
                let flows = channel_to_flowid_map.get(src_dst_pair).unwrap();
