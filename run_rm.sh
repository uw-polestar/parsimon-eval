#!/bin/bash
# sleep 6h
cur_dir=`pwd`
OUTPUT_PATH='/data1/lichenni/m3/parsimon-eval/expts/fig_8/data_dctcp'
# OUTPUT_PATH="/data2/lichenni/path/input"

# cd $OUTPUT_PATH
for shard in {0..25..1}
# for shard in 0
do
    COMMAND="rm -rf ${OUTPUT_PATH}/${shard}/mlsys_s4_last"
    # COMMAND="mv ${OUTPUT_PATH}/${shard}/mlsys ${OUTPUT_PATH}/${shard}/mlsys_e240"
    # COMMAND="mv ${OUTPUT_PATH}/${shard}/mlsys_final_reprod_v0 /data2/lichenni/data_10m/${shard}/"
    # COMMAND="mv /data2/lichenni/data_10m_/${shard}/* /data2/lichenni/data_10m/${shard}/mlsys_s2_bt100"
    # COMMAND="mkdir /data2/lichenni/data_10m/${shard}"
    # COMMAND="rm ${OUTPUT_PATH}/${shard}/ns3-config/flows.txt"
    echo "$COMMAND">>"$cur_dir/rm.log"
    ${COMMAND}>>"$cur_dir/rm.log"
    echo "">>"$cur_dir/rm.log"
done